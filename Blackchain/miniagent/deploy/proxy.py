...
def solved():
    chall_addr = open("/deploy.txt", "r").read().strip()
    user_addr = open("/user.txt", "r").read().strip()
    (result,) = abi.decode(
        ["bool"],
        web3.eth.call(
            {
...
async def proxy_request(
    ...
    instance_host = f"http://127.0.0.1:8545"
    ...
    try:
        ...
        logging.error(
            "failed to proxy anvil request to %s/%s", exc_info=e
        )
